...
export function meta(): MetaDescriptor[] {
  return [
    { title: "How to use Xtiitch · Dashboard guide" },
    {
      name: "description",
      content:
        "A plain guide to every page of the Xtiitch business dashboard — what each one is for and how to use it. Read it, or listen.",
    },
  ];
}
    ...
    >
      <Container sx={{ py: { xs: 4, md: 7 }, maxWidth: "md" }}>
        ...
        >
          {HELP_GUIDES.map((guide) => (
            <Button
              ...
              size="small"
              variant="outlined"
              sx={{ borderColor: alpha(tokens.ink, 0.15), color: "text.primary" }}
            >
              {guide.title}
            </Button>
          ))}
        ...
        <Stack spacing={2.5} sx={{ mt: 4 }}>
          {HELP_GUIDES.map((guide) => (
            <Box key={guide.section} id={guide.section} sx={{ scrollMarginTop: 24 }}>
