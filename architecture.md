...
- Migrations advanced to **`000084`**; ordering rule reaffirmed (always number above the highest ever committed — golang-migrate silently skips lower numbers).

### 1c. Hardening pass (July 2026 — codebase audit fixes)

A four-dimension audit (money, auth/RLS/tenancy, orders/storefront, concurrency) drove the following fixes. Tenant isolation was found sound (transaction-scoped RLS session vars, token-derived scope); the rest are corrections + defense-in-depth.

- **AI writing add-on made safe** — a **capability gate** (not sellable where no AI provider is configured; would otherwise charge for a passthrough no-op) plus an **admin master switch** (`admin_platform_settings.ai_assistant_addon_enabled`, migration 000081) that hides the purchase, blocks checkout, and stops renewals platform-wide (overrides everything, but can never turn a no-op deployment sellable). The add-on renewal sweep no longer double-charges: a **deterministic per-period charge reference** + an **optimistic advance guard** (`ExpectedNextChargeAt`), and a transport error now **DEFERS** (leaves the row due for a same-reference retry) instead of wrongly revoking access.
- **Checkout note bug (revenue loss)** — the storefront always sent a customer note but the API rejected the unknown field, failing **every** note-bearing order with a 400. Now accepted on all four checkout bodies and stored on `orders.note` (migration 000082).
- **Payments / settlement corrections** — payments carry `settle_amount_minor` (the order portion net of any buyer-borne fee, migration 000083) so a pass-to-buyer fee no longer inflates the order's `settled_minor` (which shorted the bespoke balance); marketplace "pay for all" now honours each shop's `FeePassToBuyer` (was silently absorbed); plan-upgrade proration **verifies the deterministic ref before charging** so a post-charge failure recovers on retry instead of stranding the tenant paid-but-not-upgraded; the recurring sweep **verifies a timed-out charge** before marking it failed (was double-charging next cycle); the webhook **reconciles the provider-reported amount** vs the expected amount (refuses to settle an underpayment); a payment **reversal now backs the amount out of the order's `settled_minor`** (Paystack refund stays a documented manual step).
- **Concurrency / data integrity** — discount-code caps are enforced **atomically under an advisory lock on the code** (counting applied + recent-pending, closing the pending→settle over-redeem race); promotion reserve is idempotent on retry (`on conflict do nothing`); customer resolve-or-create is atomic under an advisory lock on the phone (no duplicate identities); `AdvanceStage` selects the order `for update`.
- **Security hardening** — **password-login lockout** on `business_users` + `admin_users` (migration 000084): lock after N consecutive bad passwords (business 10 / admin 5) for 15 min, checked before the password compare, cleared on success, `429 account_locked`. The insecure-prod-config guard now fires for **any non-local** `APP_ENV` (not just the exact string `production`), so a `prod`/`staging` typo can't ship the default JWT signing key. Boot **asserts the DB role cannot BYPASS RLS** in non-local envs and refuses to start otherwise (guards against prod-as-DB-owner collapsing tenant isolation).
