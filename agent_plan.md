...
- Next recommended feature: the dashboard orders board (the business's working view of incoming standard/custom orders with stage advancement), then bookings/home-visit scheduling and delivery/dispatch, balance collection (payment links for the custom-order balance), and measurement-field management. Size-band charts and visit/shop measurement entry come with those.
- The backend slices (auth completion, money rails, RLS hardening, catalogue/storefront, Cloudinary), the web storefront, the dashboard, the orders/tracking view, online checkout, and the custom-order backend are committed locally. The two source PDFs are intentionally left untracked (Strictly Confidential).
...
- Implement custom-order checkout flows: measurement-by-business, measurement-by-customer, customer-confirmed saved measurements, deposit rules, and balance collection.
- Build the dashboard orders board for online and walk-in orders, including filters, stage advancement, payment state, and customer contact context.
