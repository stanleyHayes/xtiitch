...
State of the backend and business dashboard (`apps/api`, Go hexagonal; `apps/dashboard`, React Router + MUI; money in integer pesewas; **Xtiitch never holds funds**). Latest feature commit on `main`: `e21d39e`. Recently shipped this session, each as one verified+committed slice:
...
4. Dashboard functional breadth: expose money tracker, booking queue/reschedule, handovers, notifications, and availability management in the polished business shell.
...
- The `apps/dashboard` business app is built and polished: login over the auth API stores tokens in an httpOnly signed cookie session; a protected `/dashboard` uses a proper workspace shell with store profile, order metrics, a richer production board, stage advancement for confirmed orders, customer contact, payment state, a design create/retire/restore studio rail, measurement-field management, and visit/shop measurement capture; logout clears the session.
...
- Next recommended backend features: subscription billing, backend admin users/auth/operator APIs, live notification provider transport, and customer-facing handover/tracking reads. Next dashboard breadth slices: money tracker, booking queue/reschedule, handovers, notification log, and availability management. The storefront custom-order form is still available but overlaps the concurrent storefront catalogue lane.
...
- Add dedicated dashboard panels for money tracker, booking queue/reschedule, handovers, notifications, and availability management.
