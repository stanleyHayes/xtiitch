...
State of the backend and business dashboard (`apps/api`, Go hexagonal; `apps/dashboard`, React Router + MUI; money in integer pesewas; **Xtiitch never holds funds**). Latest feature commit on `main`: `1041db1`. Recently shipped this session, each as one verified+committed slice:
...
4. Dashboard role-aware permissions and staff task views once business-user roles are formalised beyond the current signed role claim.
...
- The `apps/dashboard` business app is built and polished: login over the auth API stores tokens in an httpOnly signed cookie session; a protected `/dashboard` uses a proper workspace shell with store profile, order metrics, reports snapshot, money desk/manual takings, visit queue management, handover desk, notification log, availability-window management, a richer production board, stage advancement for confirmed orders, customer contact, payment state, a design create/retire/restore studio rail, measurement-field management, and visit/shop measurement capture; logout clears the session.
...
- Next recommended backend features: subscription billing, backend admin users/auth/operator APIs, live notification provider transport, and customer-facing handover/tracking reads. Next dashboard slices should move into role-aware permissions/staff task views; the operational panels and first reporting snapshot are now built. The storefront custom-order form is still available but overlaps the concurrent storefront catalogue lane.
...
- Add dashboard role-aware permissions and staff task views once business-user roles are formalised beyond the current signed role claim.
