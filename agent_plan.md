...
2. Start the referral engine on top of the shared promotion-redemption ledger.
3. Add Paystack ad-payment collection for sponsored placements.
4. Add automatic subscription billing mechanics: Paystack recurring authorization/webhook reconciliation, grace expiry automation, downgrade rules, and plan-limit enforcement.
5. Configure production notification provider credentials/templates and run an end-to-end provider sandbox smoke for `NOTIFICATION_TRANSPORT=http`.
