...
2. Validate Paystack recurring authorization capture in provider sandbox for businesses that will use automatic package billing. Recurring charge initiation for saved authorization refs, webhook reconciliation, grace-expiry automation, downgrade rules, and design plan-limit enforcement now exist.
...
- Subscription invoice/payment-link/manual billing controls, recurring charge initiation for saved Paystack authorization refs, operator-run and worker-scheduled dunning/grace-expiry sweeps, cancellation-to-free downgrade rules, subscription webhook reconciliation, and catalogue design plan-limit enforcement now exist; production Paystack recurring authorization capture/sandbox validation is still pending.
