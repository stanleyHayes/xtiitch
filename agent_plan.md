...
- Next recommended feature: finish Milestone 2 — Cloudinary signed image uploads (the `MediaStore` port), measurement fields and size-band charts, and the `apps/web` business dashboard screens (login + catalogue management over the protected API). After that, Milestone 3 (Ordering, Stages, and Tracking).
