...
- Admin notifications, reports, operations health, launch readiness, and authenticated server-side CSV export/download endpoints are API-backed for the current admin datasets; keep adding rows as new admin features ship.
...
- Finish business-user hardening: email invites, owner-transfer policy, and continue service-level role enforcement across remaining protected API boundaries beyond auth/team/measurement/availability/password-reset/catalogue-management/media-signing/money-management/order-money.
