...
1. Add referral code issuance UI and storefront reward preview/application polish.
2. Run a Paystack provider sandbox smoke for subscription authorization capture and recurring package charges with real test credentials. The admin capture/verify flow, recurring charge initiation for saved authorization refs, webhook reconciliation, grace-expiry automation, downgrade rules, and design plan-limit enforcement now exist.
...
- Frontend stack-boundary continuation: ADR 0004 now resolves the MUI/React Router vs Expo/React Native tension by keeping dedicated web surfaces on React Router/MUI, future native apps on Expo/React Native, and shared packages limited to platform-neutral contracts, schemas, tokens and helpers. Verified with `git diff --check`.
- Subscription invoice/payment-link/manual billing controls, Paystack recurring authorization capture/verify controls, recurring charge initiation for saved authorization refs, operator-run and worker-scheduled dunning/grace-expiry sweeps, cancellation-to-free downgrade rules, subscription webhook reconciliation, and catalogue design plan-limit enforcement now exist; production Paystack sandbox validation is still pending.
- GraphQL decision continuation: ADR 0003 now confirms REST-first v1 client contracts, keeps GraphQL out of launch scope, and defines the post-v1 rule that GraphQL can be introduced only as a read-only composed read-model layer over application/query services. Verified with `git diff --check`.
- Rerun `pnpm sonar` with real `SONAR_HOST_URL`, `SONAR_ORGANIZATION`, and `SONAR_TOKEN` to complete the external quality-gate smoke.
- Mobile and generated-contract tests are still placeholder scripts until those shells/contracts have meaningful first flows.
...
- Admin notifications, reports, operations health, launch readiness, and authenticated server-side CSV export/download endpoints are API-backed for the current admin datasets, including customers; keep adding rows as new admin features ship.
...
- Growth & monetisation layer (see `## Growth & Monetisation`): plan-package CRUD, admin promotion-definition CRUD, scoped checkout redemption/application, business promotion controls, admin sponsored-placement campaign controls, sponsored-placement payment collection, public sponsored placement rendering/event tracking, admin affiliate registry/reporting, the admin referral-programme definition registry, admin referral-code issuance, public affiliate-click ingestion, checkout affiliate-attribution reservation, webhook conversion materialisation, affiliate payout reconciliation, public referral-code resolution, checkout referral-attribution reservation, webhook referral qualification/voiding, admin-triggered referral reward issuing, admin refund/dispute reversal, and storefront reward preview/application polish now exist — businesses act on their own customers; admin acts on businesses and customers; all delivered via discount-at-charge / commission-rebate / vouchers or Xtiitch-owned ad revenue so the no-funds-held invariant holds.
