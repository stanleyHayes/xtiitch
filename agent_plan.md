...
| ------------------ | ------------------------- | ---------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Admin dashboard    | `apps/admin`              | Xtiitch platform operators                     | Manage platform operators, roles, settings, business verification, money rails, risk/support, health, notifications, reports, exports, subscriptions, promotions, and audit trail | Admin auth, operator user management, role/permission editing, profile, settings, notification preferences, audit log reads, business verification decisions, business suspension/reactivation, platform metrics, money-rails read/replay/hold controls, subscription lifecycle read/update controls, plan-package create/update/archive controls, promotion create/update/archive controls, risk-review close/reopen controls, and support queue assignment/resolution are API-backed; health, notifications, reports, and exports are derived from the loaded admin read models |
...
| Backend/worker     | `apps/api`, `apps/worker` | Shared system layer                            | Tenant-safe API, payments, catalogue, orders, notifications, and background jobs                                                                      | Built in slices; admin auth/users/roles/settings/audit/business verification/business lifecycle/platform metrics/money-rails/subscription lifecycle/plan-package/promotion/risk-review/support controls are started; notification outbox has dry-run and HTTP provider transports; extra local admin bootstrap users can be seeded through env JSON; production notification provider credentials are pending                                                                                                                                                  |
...
Uncommitted admin notification/reporting/export continuation: the admin shell now has Notifications, Reports, and Exports sections without adding another backend table. Notifications derive action alerts from API-backed verification cases, failed/replayed webhooks, payout reviews, open risk reviews, support tickets, payment health, and the current operator's routing preferences. Reports derive a compliance/operator posture from the same loaded admin read models: GMV/commission/payment health, KYC backlog, payout holds, failed webhooks, open risk/support, audit posture, and platform policy, with a local CSV download for the current report. Exports adds local CSV downloads for report posture, businesses, verification, money rails, risk reviews, support tickets, audit trail, and admin users, each with jump actions back to the owning workflow. These sections use the existing fixed admin rail, `DM Serif Display` headings, Instrument Sans UI/body, and token-free SSR. Verified with `pnpm --filter @xtiitch/admin check`, `pnpm exec eslint apps/admin --max-warnings=0`, `pnpm --filter @xtiitch/admin build`, `git diff --check`, and a live admin login/dashboard smoke on `http://127.0.0.1:3403` confirming Reports/Notifications/Exports nav render with token-free HTML.
...
- Demo Postgres runs in docker container `xtiitch-demo-pg` on `localhost:5450` (db/owner `xtiitch`/`xtiitch`, app role `xtiitch_app`). Migrations `000001`–`000024` are applied locally.
...
2. Decide and implement the live subscription charging mechanics: Paystack recurring authorization vs invoice/payment-link billing, webhook reconciliation, invoice references, grace expiry automation, downgrade rules, and plan-limit enforcement.
...
- Admin dashboard work has started: `apps/admin` now runs as a standalone React Router + MUI app for `admin.xtiitch.com`, with a redesigned operator login/control-room entry, API-backed admin auth/session handling, permission-backed operator user management, editable role/permission grants, persisted profile/settings/notification preferences, a durable audit log, API-backed business verification decisions, API-backed business list/suspend/reactivate controls, API-backed platform metrics, API-backed money-rails monitoring/replay/settlement-review holds, API-backed subscription lifecycle controls, API-backed plan-package create/update/archive controls, API-backed risk-review close/reopen controls, API-backed support queue assignment/resolution, derived notifications/reports/exports/health, and a responsive operator rail/drawer.
...
- Decide exact live subscription-charging mechanics in Paystack: recurring authorization, invoice/payment-link billing, manual fallback, or a combination; lifecycle state/events are now persisted, but automatic charging/reconciliation and plan-limit enforcement are still pending.
...
- Admin notification, reporting, and local CSV export surfaces exist as derived UI over the backend-backed admin read models; server-side export/download endpoints can be added later if product review requires them.
...
- Growth & monetisation layer (see `## Growth & Monetisation`): plan-package CRUD, admin promotion-definition CRUD, and store-scoped checkout redemption/application now exist; remaining slices are business-side promotion controls, collection/design-scoped checkout targeting, a two-sided referral engine, an admin-run affiliate programme, and sponsored placements on the marketing site — businesses act on their own customers; admin acts on businesses and customers; all delivered via discount-at-charge / commission-rebate / vouchers so the no-funds-held invariant holds.
...
Map to the existing admin RBAC (`internal/domain/admin/admin.go`: roles Owner/Operator/Support, permissions): add admin permissions `manage_plans`, `manage_promotions`, `manage_ads` (or one `manage_growth`). Business-side surfaces use the business JWT + tenant RLS; admin-side surfaces use admin auth + the new permissions and write an audit event for every change.
