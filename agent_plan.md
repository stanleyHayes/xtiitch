...
State of the backend (`apps/api`, Go, hexagonal: domain → application+ports → adapters inbound-http/outbound-postgres; chi, pgx; money in integer pesewas; **Xtiitch never holds funds**). Latest feature commit on `main`: `49f9b57`. Recently shipped this session, each as one verified+committed slice:
...
1. The worker-side outbox drain/send (TS + BullMQ in `apps/worker`, different stack) — consumes `outbound_messages`, claims due rows, sends WhatsApp/SMS, and records delivery state.
2. Subscriptions — the platform's own plan billing for businesses (plans exist in `000001`).
3. Admin operator backend (admin auth + operator APIs; the `apps/admin` UI is currently mock-backed).
4. Measurement-field management and visit/shop measurement entry in the dashboard/API.
...
- The `apps/web` business dashboard is built and verified: login over the auth API stores tokens in an httpOnly signed cookie session; a protected `/dashboard` shows the store profile, order metrics, a filterable orders board, stage advancement for confirmed orders, customer contact, payment state, and the design create/retire/restore studio rail; logout clears the session.
...
- Admin dashboard frontend work has started: `apps/admin` now runs as a standalone React Router + MUI app for `admin.xtiitch.com`, with a redesigned operator login/control-room entry, a protected local operator session, platform metrics, payment-verification queue, tenant/business management with tenant inspection, Paystack money-rail monitoring, payout holds, risk review, support assignment, and an audit log that records local operator actions. This first UI layer is mock-backed by realistic operator data while backend admin users, audit logs, and operator APIs are still pending.
- The transactional notification outbox now covers order confirmation/fulfilment plus booking confirmation, balance payment, and handover dispatched/completed milestones. The worker-side outbox drain/send is still pending.
- Next recommended backend features: worker-side outbox drain/send, subscription billing, backend admin users/auth/operator APIs, and measurement-field management with visit/shop measurement entry. The storefront custom-order form is still available but overlaps the concurrent storefront catalogue lane.
...
- Implement the worker-side outbox drain/send and per-message delivery state transitions.
