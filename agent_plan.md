...
State of the backend and business dashboard (`apps/api`, Go hexagonal; `apps/dashboard`, React Router + MUI; money in integer pesewas; **Xtiitch never holds funds**). Latest feature commit on `main`: `d634071`. Recently shipped this session, each as one verified+committed slice:
...
4. Dashboard reporting/analytics once real data density grows: daily revenue, stage throughput, overdue visit follow-ups, and handover ageing.
...
- The `apps/dashboard` business app is built and polished: login over the auth API stores tokens in an httpOnly signed cookie session; a protected `/dashboard` uses a proper workspace shell with store profile, order metrics, money desk/manual takings, visit queue management, handover desk, notification log, availability-window management, a richer production board, stage advancement for confirmed orders, customer contact, payment state, a design create/retire/restore studio rail, measurement-field management, and visit/shop measurement capture; logout clears the session.
...
- Next recommended backend features: subscription billing, backend admin users/auth/operator APIs, live notification provider transport, and customer-facing handover/tracking reads. Next dashboard slices should move into reporting/analytics or reminder triage; the operational money/booking/handover/notification/availability panels are now built. The storefront custom-order form is still available but overlaps the concurrent storefront catalogue lane.
...
- Add dashboard reporting/analytics for daily revenue, stage throughput, overdue visit follow-ups, and handover ageing.
