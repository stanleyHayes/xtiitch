...
1. Add Paystack ad-payment collection for sponsored placements.
2. Add referral code issuance UI and storefront reward preview/application polish.
...
- Growth & monetisation layer (see `## Growth & Monetisation`): plan-package CRUD, admin promotion-definition CRUD, scoped checkout redemption/application, business promotion controls, admin sponsored-placement campaign controls, public sponsored placement rendering/event tracking, admin affiliate registry/reporting, the admin referral-programme definition registry, public affiliate-click ingestion, checkout affiliate-attribution reservation, webhook conversion materialisation, affiliate payout reconciliation, public referral-code resolution, checkout referral-attribution reservation, webhook referral qualification/voiding, and admin-triggered referral reward issuing now exist; remaining slices are Paystack ad-payment collection for sponsored placements, referral code issuance UI, refund/dispute reversal, and storefront reward preview/application polish — businesses act on their own customers; admin acts on businesses and customers; all delivered via discount-at-charge / commission-rebate / vouchers or Xtiitch-owned ad revenue so the no-funds-held invariant holds.
