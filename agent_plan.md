...
- Demo Postgres runs in docker container `xtiitch-demo-pg` on `localhost:5450` (db/owner `xtiitch`/`xtiitch`, app role `xtiitch_app`). Migrations `000001`–`000036` are applied locally.
...
1. Add referral reward issuing/generation on top of the shared promotion-redemption ledger.
2. Add Paystack ad-payment collection for sponsored placements.
3. Add automatic subscription billing mechanics: Paystack recurring authorization/webhook reconciliation, grace expiry automation, downgrade rules, and plan-limit enforcement.
4. Configure production notification provider credentials/templates and run an end-to-end provider sandbox smoke for `NOTIFICATION_TRANSPORT=http`.
...
- Growth & monetisation layer (see `## Growth & Monetisation`): plan-package CRUD, admin promotion-definition CRUD, scoped checkout redemption/application, business promotion controls, admin sponsored-placement campaign controls, public sponsored placement rendering/event tracking, admin affiliate registry/reporting, the admin referral-programme definition registry, public affiliate-click ingestion, checkout affiliate-attribution reservation, webhook conversion materialisation, affiliate payout reconciliation, public referral-code resolution, checkout referral-attribution reservation, and webhook referral qualification/voiding now exist; remaining slices are referral reward issuing/generation and Paystack ad-payment collection for sponsored placements — businesses act on their own customers; admin acts on businesses and customers; all delivered via discount-at-charge / commission-rebate / vouchers or Xtiitch-owned ad revenue so the no-funds-held invariant holds.
...
3. **Referral engine (two-sided).** Tables `referral_codes` (`owner_type` customer|business, `owner_id`, stable unique `code`, `status`), `referrals` (`referral_code_id`, referrer, referee, `status` pending→qualified→rewarded|void, `qualifying_order_id`; **unique referee** — referred once ever), `referral_rewards` (`referral_id`, beneficiary, `reward_kind` voucher|commission_rebate, `voucher_id` → a generated single-use promotion). Attribution is first-touch, locked at the referee's first interaction/order. **Reward triggers on the referee's first *paid* order** (`charge.success`), after a refund/dispute hold window. Delivery: referee → single-use voucher; referrer-is-customer → voucher; referrer-is-business → commission rebate (or voucher). Fraud: dedupe on **MoMo/phone number** (primary identity in Ghana), reject self-referral, per-referrer reward cap, void on refund inside the hold window. Permission: a business runs referral for **its own** customers (business-funded vouchers); admin runs platform/customer programmes and **business→business** referral (rewarded by commission rebate). Customers are platform-global (no account), so a customer's referral identity is global while the voucher it earns is scoped to the store where it redeems. **Status:** admin referral programme definition schema/API/UI registry is built; referral code execution tables are in place; public referral-code resolution, checkout referral-attribution reservation, self-referral guard, unique-referee guard, and webhook pending→qualified/void transitions are built. Referral code issuance UI, reward generation/issuing, refund/dispute reversal, and storefront application/preview polish remain pending.
