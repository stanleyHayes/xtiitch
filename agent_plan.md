...
- Growth & monetisation layer (see `## Growth & Monetisation`): plan-package CRUD, admin promotion-definition CRUD, store-scoped checkout redemption/application, and admin sponsored-placement campaign controls now exist; remaining slices are business-side promotion controls, collection/design-scoped checkout targeting, a two-sided referral engine, an admin-run affiliate programme, and public sponsored placement rendering/event tracking on the marketing site — businesses act on their own customers; admin acts on businesses and customers; all delivered via discount-at-charge / commission-rebate / vouchers or Xtiitch-owned ad revenue so the no-funds-held invariant holds.
