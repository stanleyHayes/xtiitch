...
Uncommitted pricing-book subscription-discounts/entitlements/CRM continuation: Added migration `000067_subscription_discounts_entitlements` with admin-managed plan entitlement feature/value tables and subscription discount code/redemption tables. Added admin API/service/repository routes for `GET/PATCH /v1/admin/plan-entitlements` and subscription discount code list/create/update/archive, extended subscription records and the authenticated `subscriptions` CSV export with owner contact, signup/renewal, store link, discount attribution, institution, and last-active CRM fields, and added the admin Subscriptions UI for discount-code management, a package entitlement matrix, and a subscriber CRM table with filters, quick contact/store actions, and CSV download. Verified with `cd apps/api && go test ./internal/application/adminauth ./internal/adapters/outbound/postgres ./internal/adapters/inbound/http/adminauth`, `pnpm --filter @xtiitch/admin check`, `pnpm exec eslint apps/admin/app/routes/admin.tsx apps/admin/app/lib/api.ts --max-warnings=0`, and `git diff --check -- apps/admin/app/routes/admin.tsx apps/admin/app/lib/api.ts apps/api/internal/adapters/inbound/http/adminauth/handler.go apps/api/internal/adapters/outbound/postgres/admin_auth_repository.go apps/api/internal/application/adminauth/service.go apps/api/internal/application/adminauth/service_test.go apps/api/internal/application/ports/admin.go infra/migrations/000067_subscription_discounts_entitlements.up.sql infra/migrations/000067_subscription_discounts_entitlements.down.sql`.
...
Marketing growth-programmes continuation: `apps/marketing` now has a public `/growth` route, top-nav/footer discovery, home-page teaser, Features callout, Pricing add-on section, and sitemap entry for promotion codes, referral rewards, affiliate links, and labelled sponsored placements. The copy is grounded in the existing growth implementation and repeats the no-wallet/no-escrow guardrails. Verified with `pnpm --filter @xtiitch/marketing check`, `pnpm exec eslint apps/marketing --max-warnings=0`, `pnpm --filter @xtiitch/marketing test`, `pnpm --filter @xtiitch/marketing build`, and `git diff --check`.
...
## Audit Snapshot — 2026-06-18 (what's done / what's left)

Audited by reading `apps/api` (37 migrations through `000037`; `internal/{domain,application,adapters}`), all of `apps/{marketing,storefront,dashboard,admin,worker,mobile}`, and git history. Working tree is clean; backend has 46 Go test files (domain + service + key real-Postgres integration). The verbose `## Current Work`, `## Completed`, and `## Opened / Pending` below remain accurate — this is the at-a-glance summary.
...
- Completed the public marketing growth-programmes surface: `/growth`, home teaser, Features/Pricing callouts, nav/footer links, and sitemap entry for promotions, referrals, affiliate tracking, and labelled sponsored placements.
...
- Local Docker SonarQube fallback continuation: `pnpm sonar:local` now starts a local SonarQube + Postgres pair from Docker Compose, generates ignored `.env.sonar.local` credentials, creates/selects the `Xtiitch Local Launch` gate, and tunes the local Sonar search heap. `pnpm sonar` now uses a temporary scanner settings file so the token is not exposed in process arguments, points SonarQube 9.9 at `tsconfig.sonar.json` for compatible TypeScript analysis, and excludes generated framework output. Verified with `pnpm sonar:local` and `pnpm sonar` returning `QUALITY GATE STATUS: PASSED`.
...
- React Router v8 future-flag continuation: admin, business dashboard, marketing, and storefront opt into the current v8 future behaviours where supported. Verified with `pnpm --filter @xtiitch/admin check && pnpm --filter @xtiitch/admin build`, `pnpm --filter @xtiitch/dashboard check && pnpm --filter @xtiitch/dashboard build`, `pnpm --filter @xtiitch/marketing check && pnpm --filter @xtiitch/marketing build`, and `pnpm --filter @xtiitch/storefront check && pnpm --filter @xtiitch/storefront build`. Note: the React Router v8 Vite Environment API warning still appears in current app builds and remains a framework-config cleanup item.
...
- Mobile checkout parity continuation: the mobile design checkout now accepts optional promo, referral, and affiliate codes and sends them through the already-built public checkout API; order confirmation also surfaces any returned discount amount. Verified with `pnpm --filter @xtiitch/mobile check`, `pnpm exec eslint apps/mobile --max-warnings=0`, `pnpm check`, `pnpm test`, and `git diff --check`.
- Launch-completion continuation (2026-06-29): audited the current dirty slice after reading this plan and carried it to clean verification. The slice now covers four-tier pricing (`free` / `starter` / `growth` / `studio`), catalogue ordering guards, size-band charts, pricing-mode/image-limit enforcement, combined ready-made cart checkout with checkout groups and one Paystack charge, delivery-zone setup and storefront delivery fees, auto handover creation on fulfilment, real-time store-handle availability on signup, Ghana Card identity-document submission/review, admin help guides with speech support, fee-free manual-taking guidance, and self-contained full demo seed catalogue rows. Also fixed source hygiene found during verification: generated `.vercel` output is ignored by ESLint and storefront cart line IDs use UUIDs so duplicate items remove one at a time. Verified with `cd apps/api && go test ./internal/...`, `cd apps/api && go vet ./...`, `cd apps/api && go build ./...`, `pnpm exec eslint apps/storefront/app apps/storefront/react-router.config.ts apps/storefront/vite.config.ts --max-warnings=0`, `pnpm exec eslint apps/dashboard/app apps/dashboard/react-router.config.ts apps/dashboard/vite.config.ts --max-warnings=0`, `pnpm exec eslint apps/admin/app apps/admin/react-router.config.ts apps/admin/vite.config.ts --max-warnings=0`, `pnpm --filter @xtiitch/storefront check`, `pnpm --filter @xtiitch/dashboard check`, `pnpm --filter @xtiitch/admin check`, `pnpm --filter @xtiitch/storefront test`, `pnpm --filter @xtiitch/dashboard test`, `pnpm --filter @xtiitch/admin test`, `pnpm --filter @xtiitch/storefront build`, `pnpm --filter @xtiitch/dashboard build`, `pnpm --filter @xtiitch/admin build`, and `git diff --check`.
...
- External SonarCloud remains pending until real `SONAR_HOST_URL`, `SONAR_ORGANIZATION`, and `SONAR_TOKEN` values are supplied; the local Docker SonarQube fallback now passes with `pnpm sonar:local` and `pnpm sonar`.
