...
- Next recommended feature: extend the dashboard with store settings, size bands, and per-band pricing UI (plus image upload via the signed endpoint and silent token refresh), then start Milestone 3 (Ordering, Stages, and Tracking) — order classification, the deposit/full-payment paths over the money rails, production stages, and the red/yellow/green "where is my cloth?" customer tracking view. Measurement fields and size-band charts are best built alongside Milestone 3's measurement capture.
- The backend slices (auth completion, money rails, RLS hardening, catalogue/storefront, Cloudinary) and the web storefront are committed (`e12d43a`, `27d74f7`, `47947a2`); the dashboard slice is green and pending commit. The two source PDFs are intentionally left untracked (Strictly Confidential).
